...
nb_epoch = tf.placeholder(tf.int32, ())
x_adv = smda(model, x, target, nb_epoch=nb_epoch)
...
if saved:
    with gzip.open('data/ex_02.pkl.gz', 'rb') as r:
    ...
    for i in np.arange(10):
        ...
        while True:
            X_i_adv = sess.run(x_adv, feed_dict={
                x: blank, target: i, nb_epoch: maxiter,
                K.learning_phase(): 0})
    ...
    with gzip.open('data/ex_02.pkl.gz', 'wb') as w:
