...
def model(x, logits=False, training=False):
    conv0 = tf.layers.conv2d(x, filters=32, kernel_size=[3, 3], padding='same',
                             name='conv0', activation=tf.nn.relu)
    pool0 = tf.layers.max_pooling2d(conv0, pool_size=[2, 2], strides=2,
                                    name='pool0')
    conv1 = tf.layers.conv2d(pool0, filters=64, kernel_size=[3, 3],
...
# Collect all tensors into one "enviroment" to avoid accidental overwriting.
class Dummy:
    pass
env = Dummy()
...
with tf.variable_scope('model', reuse=True):
    ...
    env.x_adv = jsma(model, env.x, env.target, pair=True, epochs=50, eps=1.0,
                     min_proba=1.1)
...
if False:
    ...
    env.saver.restore(sess, "data/ex_04.ckpt")
else:
    print('\nTraining')
    ...
    env.saver.save(sess, 'data/ex_04.chpt')
...
if False:
    ...
    os.makedirs('data', exist_ok=True)
    np.savez('data/ex_04.npy', X_adv=X_adv, proba=proba)
